...
import ingestTableRoutes from './routes/ingestTable.js';
...
// tableRoutes registers literal paths under /api/now/table/ (sys_dictionary,
// sys_choice) alongside the outbound v2 incident endpoints. It must be
// mounted BEFORE ingestTableRoutes, whose /api/now/table/:table is a
// catch-all param route for the same /api/now/table/ prefix — Express
// matches whichever route was registered first, so this order keeps the
// literal paths from being shadowed by the generic ingest route.
app.use(tableRoutes);
app.use(ingestTableRoutes);
...
function startServer() {
  ...
  server.listen(PORT, () => {
    ...
    console.log(`\n  Configure the self-managed ServiceNow INGEST connector with:`);
    console.log(`    url: ${protocol}://localhost:${PORT}`);
    console.log(`    services: *`);
    console.log(`    use_document_level_security: false`);
    if (NO_TLS) {
